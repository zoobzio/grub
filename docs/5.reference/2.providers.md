...
```go
import "github.com/zoobzio/grub/pgvector"
```
...
provider := pgvector.New(db, pgvector.Config{
    Table:          "vectors",
    IDColumn:       "id",
    VectorColumn:   "embedding",
    MetadataColumn: "metadata",
    Distance:       pgvector.L2,
})
```
...
type Config struct {
    ...
    VectorColumn   string         // Vector column name (default: "embedding")
    MetadataColumn string         // Metadata JSONB column (default: "metadata")
    Distance       DistanceMetric // L2, Cosine, or InnerProduct
}
```
...
const (
    ...
    InnerProduct DistanceMetric = "inner_product" // Inner product (negative)
)
```
...
| SQL integration | Full |

#### Error Mapping

| PostgreSQL Error | Grub Error |
|------------------|------------|
| No rows | `ErrNotFound` |
...
- Table must exist with vector column
- Uses JSONB for metadata storage
- Supports all SQL-compatible filter operations
